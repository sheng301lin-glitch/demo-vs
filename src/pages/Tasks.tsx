...
function getTaskCost(status: string, cost: number | null | undefined) {
  if (cost != null && cost > 0) return formatCNY(cost)
  return ['RUNNING', 'QUEUED', 'PENDING'].includes(status) ? '—' : '¥0.00'
}

function getStatusDescription(status: string) {
  if (status === 'SUCCESS') return '任务已成功执行并完成'
  if (status === 'FAILED') return '任务执行失败，可查看事件后重试'
  if (['PENDING', 'QUEUED'].includes(status)) return '任务正在等待 Worker 处理'
...
export function TasksPage() {
  ...
  const tasksQuery = useQuery({ queryKey: ['tasks', status, currentNode, platform, priority, keyword, page, pageSize], queryFn: () => fetchTasks({ status, current_node: currentNode, platform: platform || undefined, priority: priority || undefined, keyword: keyword || undefined, page, size: pageSize }), refetchInterval: query => query.state.data?.data?.items.some(task => ['PENDING', 'QUEUED', 'RUNNING'].includes(task.status)) ? 3000 : false })
  ...
  const detailTokens = (detail?.usage_summary?.total_input_tokens ?? 0) + (detail?.usage_summary?.total_output_tokens ?? 0)
  const inputLines = useMemo(() => formatJsonLines(detail?.input_params), [detail?.input_params])
    ...
    <div>
      ...
      <DetailModal open={!!selectedId} title="任务详情" onClose={closeTask} size="task">{detailQuery.isLoading ? <div className="empty-state"><Loader2 size={28} /></div> : detail ? <div className="task-detail-layout">
        ...
        <section className="task-overview-grid">
          <dl className="task-info-table"><dt>任务 ID</dt><dd>{detail.task_id}</dd><dt>平台</dt><dd>{getPlatformLabel(detail.platform)}</dd><dt>优先级</dt><dd>{getPriorityLabel(detail.priority)}</dd><dt>等待时长</dt><dd>{formatDuration(detail.queue_wait_ms)}</dd><dt>模型</dt><dd>{detail.model_summary?.length ? detail.model_summary.join('、') : '暂无调用'}</dd><dt>Token</dt><dd>{formatTokens(detailTokens)}</dd><dt>费用（人民币）</dt><dd>{formatCNY(detail.usage_summary?.total_cost)}</dd></dl>
          <div className="task-summary-cards"><article className="task-summary-card"><div className="task-summary-icon"><CheckCircle2 size={28} /></div><div><span>任务状态</span><b>{statusInfo?.label ?? detail.status}</b><p>{getStatusDescription(detail.status)}</p></div></article><article className="task-summary-card"><div className="task-progress-ring" style={{ background: `conic-gradient(var(--primary) ${detail.progress * 3.6}deg,#eceef8 0deg)` }}><i>{detail.progress}%</i></div><div><span>完成进度</span><b>{detail.progress}%</b><p>{detail.success_count >= detail.requested_count ? '所有内容已生成完成' : `${detail.success_count}/${detail.requested_count} 已完成`}</p></div></article></div>
        </section>

        {detail.input_params && <section className="task-section"><button type="button" className="task-section-toggle" aria-expanded={paramsExpanded} onClick={() => setParamsExpanded(value => !value)}><Braces size={16} />请求参数{paramsExpanded ? <ChevronDown size={16} /> : <ChevronRight size={16} />}</button>{paramsExpanded && <div className="task-code-block"><button className="icon-button" aria-label="复制请求参数"><Copy size={14} /></button>{inputLines.map((line, index) => <div className="task-code-line" key={`${index}-${line}`}><span>{index + 1}</span><code>{line}</code></div>)}</div>}</section>}
