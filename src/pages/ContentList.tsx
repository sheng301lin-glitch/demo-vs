...
export function ContentListPage() {
  ...
  return <div className="page">
    ...
    <div className="split-workspace">
      <section className="card"><div className="table-wrap">{groupsQuery.isLoading ? <div className="empty-state"><Loader2 size={28}/><div>加载内容...</div></div> : groups.length === 0 ? <div className="empty-state"><Layers3 size={36}/><div>暂无符合条件的内容</div></div> : <table className="data-table content-table"><thead><tr><th>内容 ID</th><th>标题</th><th>平台</th><th>来源任务</th><th>当前版本</th><th>历史版本数</th><th>评分</th><th>状态</th><th>更新时间</th></tr></thead><tbody>{groups.map((group: ContentGroupDetail) => <tr key={group.content_group_id} className={selectedId === group.content_group_id ? 'is-selected' : ''} onClick={() => setSelectedId(group.content_group_id)}><td>{group.content_group_id}</td><td><b>{group.current_content?.title || '无标题'}</b></td><td>{PLATFORMS.find(p => p.value === group.platform)?.label ?? group.platform}</td><td>{group.root_task_id}</td><td>v{group.current_version_no}</td><td>{group.version_count}</td><td>{group.current_content?.score != null ? <span className={group.current_content.score >= 80 ? 'badge green' : 'badge orange'}>{group.current_content.score}</span> : '—'}</td><td><span className={`badge ${group.status === 'ACTIVE' ? 'green' : 'red'}`}>{group.status === 'ACTIVE' ? (group.version_count > 1 ? '已优化' : '有效') : '已归档'}</span></td><td>{group.updated_at ? new Date(group.updated_at).toLocaleString('zh-CN') : '-'}</td></tr>)}</tbody></table>}</div><div className="pagination"><button disabled={page === 1} onClick={() => setPage(page - 1)}>‹</button><button className="is-active">{page}</button><button disabled={page * PAGE_SIZE >= total} onClick={() => setPage(page + 1)}>›</button></div></section>
      {selectedId && <button className="detail-backdrop" aria-label="关闭内容详情" onClick={() => setSelectedId(null)} />}
      <aside className={`card detail-panel ${selectedId ? 'is-open' : ''}`}>{!selectedId ? <div className="empty-state"><FileText size={38}/><div>选择内容查看详情和历史版本</div></div> : detailQuery.isLoading ? <div className="empty-state"><Loader2 size={28}/></div> : detail ? <><div className="card-header"><span>内容详情</span><button className="icon-button" onClick={() => setSelectedId(null)}>×</button></div><div className="card-body"><h3 className="detail-title">{current?.title || '无标题'}</h3><p><span className="badge green">{detail.status === 'ACTIVE' ? '有效' : '已归档'}</span></p><dl className="detail-meta"><dt>内容 ID</dt><dd>{detail.content_group_id}</dd><dt>当前版本</dt><dd>v{detail.current_version_no}</dd><dt>平台</dt><dd>{detail.platform}</dd><dt>模型</dt><dd>{current?.model_name || '—'}</dd><dt>综合评分</dt><dd>{current?.score ?? '—'}</dd></dl><h4>内容预览</h4><p style={{ whiteSpace:'pre-wrap',fontSize:12,lineHeight:1.8,color:'#596276' }}>{current?.body || '暂无正文'}</p>{current?.evaluation_detail && <><h4>评分维度</h4>{Object.entries(current.evaluation_detail).map(([key,value]) => typeof value === 'number' ? <div key={key} style={{ display:'grid',gridTemplateColumns:'75px 1fr 35px',gap:8,alignItems:'center',fontSize:10,marginBottom:7 }}><span>{key}</span><div className="progress" style={{ width:'100%' }}><i style={{ width:`${Math.min(100,value)}%` }}/></div><b>{value}</b></div> : null)}</>}</div><div className="card-body" style={{ borderTop:'1px solid var(--line)' }}><h4>历史版本</h4><div className="timeline">{versions.map(version => <div className="timeline-item" key={version.content_id}><b>v{version.version_no} · {version.score ?? '未评分'}</b><div className="kpi-hint">{version.created_at ? new Date(version.created_at).toLocaleString('zh-CN') : ''}</div><button className="button" style={{ marginTop:6,height:29 }} onClick={() => startOptimize(version.content_id)} disabled={optimize.isPending}><Wand2 size={12}/>从此版本优化</button></div>)}</div><textarea className="textarea" value={instruction} onChange={e => setInstruction(e.target.value)} placeholder="输入优化要求，例如：标题更抓眼，正文更口语化" style={{ marginTop:14 }}/><div style={{ display:'flex',gap:8,marginTop:10 }}><button className="button danger" disabled={detail.status === 'ARCHIVED'} onClick={() => archive.mutate(detail.content_group_id)}><Archive size={12}/>归档</button>{current && <button className="button primary" onClick={() => startOptimize(current.content_id)} disabled={optimize.isPending}><Wand2 size={12}/>优化当前版本</button>}</div></div></> : <div className="empty-state"><XCircle size={32}/>内容详情加载失败</div>}</aside>
    </div>
